...
public final class RemnantComponentImpl implements RemnantComponent {
    ...
    public boolean canSplitPlayer(boolean ignoreGameConditions) {
        var notRemoved = !this.player.isRemoved();
        var canSplit = this.state.canSplit(ignoreGameConditions);
        var notVagrant = !this.isVagrant();
        var test = (ignoreGameConditions || PlayerShellEvents.PRE_SPLIT.invoker().canSplit(this.player));
        return notRemoved && canSplit && notVagrant && test;
    }
